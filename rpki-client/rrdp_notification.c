...
check_state(struct notification_xml *nxml)
{
	...
	if (serial_diff < 0) {
		...
		warnx("serial_diff is negative %d vs %d",
...
{
	logx("scope: %d", notification_xml->scope);
	logx("state: %d", notification_xml->state);
	logx("xmlns: %s", notification_xml->xmlns ?: "NULL");
	logx("version: %d", notification_xml->version);
	logx("current_session_id: %s",
	    notification_xml->current_session_id ?: "NULL");
	logx("current_serial: %d", notification_xml->current_serial);
	logx("session_id: %s", notification_xml->session_id ?: "NULL");
	logx("serial: %d", notification_xml->serial);
	logx("snapshot_uri: %s", notification_xml->snapshot_uri ?: "NULL");
	logx("snapshot_hash: %s",
