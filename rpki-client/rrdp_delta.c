...
{
	logx("scope: %d", delta_xml->scope);
	logx("xmlns: %s", delta_xml->xmlns ?: "NULL");
	logx("version: %d", delta_xml->version);
	logx("session_id: %s", delta_xml->session_id ?: "NULL");
	logx("serial: %d", delta_xml->serial);
}
...
write_delta(struct xmldata *xml_data, int withdraw)
{
	...
	if (withdraw == 0) {
		/* decode b64 message */
		if (base64_decode(delta_xml->publish_data, &data_decoded,
		    &decoded_len) == -1) {
